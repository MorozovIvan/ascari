...
class ControllerDesignBanner extends Controller {
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$last_banner_image_id=$this->model_design_banner->addBanner($this->request->post);
			...
			$this->redirect($this->url->link('design/banner/update', 'token=' . $this->session->data['token'] . $url.'&banner_id='.$last_banner_image_id, 'SSL'));
		}
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->redirect($this->url->link('design/banner/update', 'token=' . $this->session->data['token'] . $url.'&banner_id='.$this->request->get['banner_id'], 'SSL'));
		}
	...
	private function getForm() {
		...
		foreach ($banner_images as $banner_image) {
			...
			$this->data['banner_images'][] = array(
                'sort_order'               => isset($banner_image['sort_order']) ? $banner_image['sort_order'] : 0,
